    ...
    if(str2==NULL||str3==NULL||str4==NULL||str5==NULL||str7==NULL||str6==NULL||str9==NULL)
     {
     ...
     else
      {
            ...
            if(query.isActive())
                ...
                this->close();
                //将仓库信息发给socket客户端
                Client *sk_client=new Client();
                sk_client->setAttribute(Qt::WA_QuitOnClose,false);//主窗口关闭时同时关闭该窗口
                sk_client->move((QApplication::desktop()->width() - sk_client->width()) / 2,
                                  (QApplication::desktop()->height() - sk_client->height()) / 2);//桌面正中
                sk_client->setWindowModality(Qt::ApplicationModal);//设置模态，禁止使用其他对话框
                sk_client->show();
                connect(this,SIGNAL(sendData(QString,QString,QString)),sk_client,SLOT(displayData2(QString,QString,QString)));
                emit sendData(str2,str3,str4);
                }
            else
