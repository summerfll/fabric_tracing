...
//{
//    socket_client *sck_client = new socket_client();
//    sck_client->setAttribute(Qt::WA_QuitOnClose,false);//主窗口关闭时同时关闭该窗口
//    sck_client->move((QApplication::desktop()->width() - sck_client->width()) / 2,
//                      (QApplication::desktop()->height() - sck_client->height()) / 2);//桌面正中
//    //sck_client->setWindowModality(Qt::ApplicationModal);//设置模态，禁止使用其他对话框
//    sck_client->show();
//}
...
void GraphicsWidget::on_pushButton_89_clicked()
{
    int click_num = 10;
    connect(this,SIGNAL(send_click11(int)),ui->tabWidget,SLOT(setCurrentIndex(int)));
    emit send_click11(click_num);
}

void GraphicsWidget::on_pushButton_4_clicked()
{
    QSqlQuery query;
    QString goods_id=ui->lineEdit_25->text().trimmed();

    query.exec("select *from fabric_data where ID = '"+goods_id+"'");
    if(query.next())
    {
        QSqlRecord record=query.record();
        QString ID=query.value(record.indexOf("ID")).toString();
        QString TxId=query.value(record.indexOf("TxId")).toString();
        QString Value=query.value(record.indexOf("Value")).toString();
        QString TimeStamp=query.value(record.indexOf("TimeStamp")).toString();
        ui->listWidget->insertItem(1,"-----------------------------商品历史信息查询-----------------------------------");
        QString data1="商品ID： ";
        QString data2="交易HASH值： ";
        QString data3="交易记录： ";
        QString data4="交易时间戳： ";
        data1=data1.append(ID);
        data2=data2.append(TxId);
        data3=data3.append(Value);
        data4=data4.append(TimeStamp);
        ui->listWidget->insertItem(2,data1);
        ui->listWidget->insertItem(3,data2);
        ui->listWidget->insertItem(4,data3);
        ui->listWidget->insertItem(5,data4);
    }
    else
    {
         QMessageBox::warning(this, tr("警告"), tr("显示失败！"));
    }

    query.exec("select *from fabric_data where ID = 111");
    if(query.next())
    {
        QString data5="交易HASH值： ";
        QString data6="交易记录： ";
        QString data7="交易时间戳： ";
        QSqlRecord record=query.record();
        QString TxId=query.value(record.indexOf("TxId")).toString();
        QString Value=query.value(record.indexOf("Value")).toString();
        QString TimeStamp=query.value(record.indexOf("TimeStamp")).toString();
        data5=data5.append(TxId);
        data6=data6.append(Value);
        data7=data7.append(TimeStamp);
        ui->listWidget->insertItem(6,"--------------------------------------------------------------------");
        ui->listWidget->insertItem(7,data5);
        ui->listWidget->insertItem(8,data6);
        ui->listWidget->insertItem(9,data7);
    }
    query.exec("select *from fabric_data where ID = 222");
    if(query.next())
    {
        QString data8="交易HASH值： ";
        QString data9="交易记录： ";
        QString data10="交易时间戳： ";
        QSqlRecord record=query.record();
        QString TxId=query.value(record.indexOf("TxId")).toString();
        QString Value=query.value(record.indexOf("Value")).toString();
        QString TimeStamp=query.value(record.indexOf("TimeStamp")).toString();
        data8=data8.append(TxId);
        data9=data9.append(Value);
        data10=data10.append(TimeStamp);
        ui->listWidget->insertItem(10,"--------------------------------------------------------------------");
        ui->listWidget->insertItem(11,data8);
        ui->listWidget->insertItem(12,data9);
        ui->listWidget->insertItem(13,data10);
    }
